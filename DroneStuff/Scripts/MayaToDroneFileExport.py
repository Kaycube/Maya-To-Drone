...
def exportCamera(Camera=None, fileName=None):
    ...
    if cmds.nodeType(Camera) == "transform":
        cameraShape = cmds.listRealatives(Camera, children=True)[0]
    ...
    if not fileName:
        #run file dialog
        basicFilter = ".json"
        retval = cmds.fileDialog2(dialogStyle=2, fn=0)
    ...
    for i in range(frameStart, frameEnd+1):
        ...
        outData["frames"]["matrix_world"][str(i)] = cmds.xform(Camera, q=True, matrix=True, worldSpace=True)
