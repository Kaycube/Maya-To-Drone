...
#		reconnect all given network-sockets.
	...
	# If the drone does not get a command, it will mutter after 50ms (CTRL watchdog / joystate[28] will set to 1)
	# and panic after 2 seconds and abort data-communication on port 5554 (then you have to initialize the network again).
	# Heartbeat will reset the watchdog and, by the way, the ACK_BIT (joystate[6], to accept any other AT*CONFIG command)
...
#Bit 24-31: ATCODEC_THREAD_ON, NAVDATA_THREAD_ON, VIDEO_THREAD_ON, ACQ_THREAD_ON, CTRL_WATCHDOG_MASK, ADC_WATCHDOG_MASK, COM_WATCHDOG_MASK, EMERGENCY_MASK
	...
	stateBit[ 5] = data[1]>> 5&1	#  5: USER feedback : 				Start button joystate
	...
	stateBit[18] = data[1]>>18&1	# 18: Magnetometer calib joystate :	(0) Ok, no calibration needed, (1) not ok, calibration needed
...
def decode_ID10(packet):		#NAVDATA_ALTITUDE_TAG
	...
	altitude[7] = dataset[11]			# obs_state			Observer joystate [-]		(uint32)
	for i in range (0,2,1):
		altitude[8][i] = dataset[12+i]	# est_vb			2-Vector				(float)
	altitude[9] = dataset[14]			# est_state			Observer flight joystate 	(uint32)
	return(altitude)
...
def getNavdata(packet,choice):
	...
	length =	len(packet)
	dataset = 	struct.unpack_from("IIII", packet, 0)	# Reading (Header, State, Sequence, Vision)
	navdata["joystate"] = decode_Header(dataset)
	offsetND = 	struct.calcsize("IIII")
